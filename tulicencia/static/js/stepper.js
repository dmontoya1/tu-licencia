                ...
                $('.crc-list').append(
                    `
                        <div class="col-12 col-xl-6">
                            <button type="button" class="company-detail" data-id="${v.id}" data-company="crc" data-toggle="modal" data-target="#companyDetailModal-${v.id}">
                        ...
                        <div class="modal fade" id="companyDetailModal-${v.id}" tabindex="-1" role="dialog" aria-labelledby="companyDetailModalTitle" aria-hidden="true">
        ...
        if (data.length > 0){
            ...
            $.each(data, function(i, v){
                ...
                $('.cea-list').append(
                    `
                        <div class="col-12 col-xl-6">
                            <button type="button" class="company-detail" data-id="${v.id}" data-company="cea" data-toggle="modal" data-target="#companyDetailModal-${v.id}">
                                <div class="d-flex flex-row content-result rounded mb-2">
                                    <div class="thumbnail mr-2 mb-4">
    ...
    .then(function (response) {
        ...
        if (data.length > 0){
            ...
            $.each(data, function(i, v){
                    ...
                    `
                        <div class="col-sm-12 col-md-6 col-lg-c-3 company-detail">
                            ...
                            <div class="row mt-2">
                                ...
                                <div class="col-12 transit-${v.id}-rating mt-3">
                ...
                const starPercentage = (v.rating / starTotal) * 100;
                const starPercentageRounded = `${(Math.round(starPercentage / 10) * 10)}%`;
                $(`.transit-${v.id}-rating .stars-outer .stars-inner`).width(starPercentageRounded);
            })
...
$('.online').on('click', function(e){
    if($(this).attr('selected')){
        ...
        if ($('.credit').attr('selected')){
            $('.credit span.imgCheckbox0').removeClass('imgChked');
            $('.credit').removeAttr('selected')
        }
        $(this).attr('selected', true)
        payment_type = 'CO'
    }
...
$('.credit').on('click', function(e){
    if($(this).attr('selected')){
        ...
        if ($('.online').attr('selected')){
            $('.online span.imgCheckbox0').removeClass('imgChked');
...
$('#licence-request-form').on('submit', function(e){
    ...
    if($('#doc_id').val() == ""){
        ...
        swal({
        title: 'Atención',
        text: 'Ingresa el de número de documento de identidad',
        type: 'error',
        showCancelButton: false,
        confirmButtonText: 'Ok'
        })
    }
    if($('#doc_type :selected').val().trim() === ""){
        submit = false
        swal({
        title: 'Atención',
        text: 'Ingresa el tipo de documento de identidad',
        type: 'error',
        showCancelButton: false,
        confirmButtonText: 'Ok'
        })
        ...
        if (payment_type == "CR"){
            ...
            .then(function (response) {
                ...
                swal({
                    ...
                    type: 'success',
                    showCancelButton: false,
                    confirmButtonText: 'Ver Baucher'
                }).then((result) => {
                    if (result.value) {
                        window.location.replace(`/baucher/${data.request}/request`);
                    }
                })
            })
            .catch(function (error) {
